...
#include "base.h"
 ...
 /*
  * Padd payload buffer to make it compatible with tiny-AES library
  *
  * @param pInputBuffer: Input buffer
  * @param pInputBufferSize: Size of input buffer
  * @param pOutputBuffer: Pointer to store padded buffer
  * @param pOutputBufferSize: Size of padded buffer
  *
...
/*
 * XOR protect encryption key with a random byte
 *
 * @param sSize: Size of the key
 * @param pbHintByte: Hint bytes which is then used for brute-force decryption
 * @param ppOriginalKey: Pointer to unprotected key
 * @param ppProtectedKey: Pointer to protected key
 * 
 */
